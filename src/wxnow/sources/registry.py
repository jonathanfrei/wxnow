...
def load_builtin() -> None:
    ...
    async def _lightning(pin: Pin, http: Http, cfg: Config):
        return await fetch_lightning(pin, http)
    ...
    register(Plugin("airnow", "AirNow", "observation", "observation", needs_key="airnow", fetch=_airnow))
    register(Plugin("sigmet", "SIGMET/AIRMET", "alerts", "hazards", fetch=_sigmet))
    register(Plugin("lightning", "Lightning", "extra", "lightning", fetch=_lightning))
